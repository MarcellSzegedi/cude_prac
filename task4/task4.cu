...
__global__ void matmul_kernel_stride(float *A, float *B, float *C, int N, int K,
                                     int M) {
  int start_row = blockDim.y * blockIdx.y + threadIdx.y;
  int start_col = blockDim.x * blockIdx.x + threadIdx.x;
  ...
  for (int row = start_row; row < N; row += row_stride) {
    for (int col = start_col; col < M; col += col_stride) {
      if (row < N && col < M) {
        float res = 0.0f;
        for (int k = 0; k < K; k++) {
          res += A[K * row + k] * B[M * k + col];
        }
        C[M * row + col] = res;
      }
    }
